...
@section('content')
        ...
        <div class="panel panel-default">
            ...
            <div class="panel-body">
                <form action="{{ URL::to('admin/product/search')}}" class="form-inline" method="post">
                    {{-- @csrf --}}
                    {{ csrf_field() }}
                    <input type="text" name="q" class="form-control" placeholder="พิมพ์รหัสหรือชื่อเพื่อค้นหา">
                    <button type="submit" class="btn btn-primary">ค้นหา</button>
                    <a href="{{ URL::to('admin/product/edit') }}" class="btn btn-success pull-right">เพิ่มสินค้า
                    </a>
                </form>
            </div>
        </div>
            ...
            <tbody>
                @foreach($products as $p)
                    <tr>
                        ...
                        <td class="bs-center">
                            <a href="{{ URL::to('admin/product/edit/'.$p->id) }}" class="btn btn-info">
    ...
    <script>
        $('.btn-delete').on('click', function() { 
            if(confirm("คุณต้องการลบข้อมูลสินค้าหรือไม่?")){
                var url = "{{ URL::to('admin/product/remove') }}" + '/' + $(this).attr('id-delete'); window.location.href = url;
            }
        });
    </script>
@endsection
